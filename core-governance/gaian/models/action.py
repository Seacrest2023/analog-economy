...
class GameState(BaseModel):
    ...
    profession: Optional[str] = Field(default=None, description="Primary profession")
    ...
    equipped_tool: Optional[str] = Field(default=None, description="Currently equipped")


class ActionDetails(BaseModel):
    """Details specific to the action being performed."""
    type: ActionType = Field(..., description="Type of action")
    target: Optional[str] = Field(default=None, description="Target object/entity ID")
    target_type: Optional[str] = Field(default=None, description="Type of target")
    position: Optional[Position] = Field(default=None, description="Action position")
...
class ActionRequest(BaseModel):
    ...
    game_time: Optional[float] = Field(
        default=None,
        description="In-game time (days since start)"
    )
    ...
    game_state: GameState = Field(
        ...
        description="Player state at action time"
    )
...
class ActionResponse(BaseModel):
    ...
    status: str = Field(
        default="approved",
        description="Action status: approved, rejected, pending"
    )

    # Reward
    sila_reward: float = Field(
        default=0.0,
        ge=0,
        description="SILA tokens earned for this action"
    )

    # Feedback
    feedback: Optional[str] = Field(
        default=None,
        description="Optional feedback message for player"
    )

    # Training data tracking
    training_data_id: UUID = Field(
        default_factory=uuid4,
        description="ID of the training record created"
    )
    ...
    quality_score: Optional[float] = Field(
        ...
        ge=0,
        le=1,
        description="Quality score of the action (0-1)"
    )
    novelty_score: Optional[float] = Field(
        default=None,
        ge=0,
        le=1,
        description="Novelty score of the action (0-1)"
    )

    # Game state updates (if action modified state)
    state_updates: dict[str, Any] = Field(
        default_factory=dict,
        description="Any game state changes to apply"
    )
